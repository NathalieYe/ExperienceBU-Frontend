...
function (_React$Component) {
  ...
  function SelectToggle(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "onEscPress", function (event) {
      ...
      if (isOpen && (event.key === _selectConstants.KeyTypes.Escape || event.key === _selectConstants.KeyTypes.Tab) && parentRef && parentRef.current.contains(event.target)) {
        onToggle(false);
        onClose();
    ...
    _defineProperty(_assertThisInitialized(_this), "onKeyDown", function (event) {
      ...
      if (event.key === _selectConstants.KeyTypes.Tab && variant === _selectConstants.SelectVariant.checkbox || event.key === _selectConstants.KeyTypes.Tab && !isOpen || event.key !== _selectConstants.KeyTypes.Enter && event.key !== _selectConstants.KeyTypes.Space || (event.key === _selectConstants.KeyTypes.Space || event.key === _selectConstants.KeyTypes.Enter) && (variant === _selectConstants.SelectVariant.typeahead || variant === _selectConstants.SelectVariant.typeaheadMulti)) {
        return;
      }

      event.preventDefault();

      if ((event.key === _selectConstants.KeyTypes.Tab || event.key === _selectConstants.KeyTypes.Enter || event.key === _selectConstants.KeyTypes.Space) && isOpen) {
        onToggle(!isOpen);
        onClose();

        _this.toggle.current.focus();
      } else if ((event.key === _selectConstants.KeyTypes.Enter || event.key === _selectConstants.KeyTypes.Space) && !isOpen) {
        onToggle(!isOpen);
        onEnter();
      }
    });
  ...
  _createClass(SelectToggle, [{
    ...
    value: function render() {
      ...
      var _this$props4 = this.props,
          ...
          props = _objectWithoutProperties(_this$props4, ["className", "children", "isOpen", "isFocused", "isActive", "isHovered", "isPlain", "isDisabled", "variant", "onToggle", "onEnter", "onClose", "handleTypeaheadKeys", "parentRef", "id", "type", "hasClearButton", "aria-labelledby", "aria-label"]);
