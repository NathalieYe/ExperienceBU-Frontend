...
export const formatBreakpointMods = (breakpointMods, styles) => breakpointMods.map(mod => `${mod.modifier}${mod.breakpoint ? `-on-${mod.breakpoint}` : ''}`).map(toCamel).map(modifierKey => styles.modifiers[modifierKey]).filter(Boolean).join(' ');
