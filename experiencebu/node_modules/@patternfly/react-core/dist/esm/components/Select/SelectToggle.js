...
export class SelectToggle extends React.Component {
  constructor(props) {
    ...
    _defineProperty(this, "onEscPress", event => {
      ...
      if (isOpen && (event.key === KeyTypes.Escape || event.key === KeyTypes.Tab) && parentRef && parentRef.current.contains(event.target)) {
        onToggle(false);
        onClose();
        this.toggle.current.focus();
      }
    ...
    _defineProperty(this, "onKeyDown", event => {
      ...
      if (event.key === KeyTypes.Tab && variant === SelectVariant.checkbox || event.key === KeyTypes.Tab && !isOpen || event.key !== KeyTypes.Enter && event.key !== KeyTypes.Space || (event.key === KeyTypes.Space || event.key === KeyTypes.Enter) && (variant === SelectVariant.typeahead || variant === SelectVariant.typeaheadMulti)) {
        return;
      }
