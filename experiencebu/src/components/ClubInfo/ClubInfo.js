...
export default class ClubInfo extends React.Component {
  constructor(props) {
    ...
    this.handleClick = () => {
      ...
      let updatedsub = this.state.user[0].subscriptions.push(this.cludID);
      ...
      if (this.state.isSignedIn) {
        if (!_.isEmpty(this.state.user)){
          if (this.state.subscribed) {
            button = <Button isDisabled='true' isBlock>Added to My Organizations</Button>;
          }
          else {
            button = <Button onClick={this.handleClick} isBlock>Add to My Organizations</Button>;
          }
        }
      }
